...
// ===========================================
(function() {
  // ✅ STARFIELD ENABLED - CM-STARFIELD-FIX-001
...
function updateTooltipBreakdown() {
  ...
  let currentTier = 0;
  if (pairCount >= 4) currentTier = 4;
  else if (pairCount >= 3) currentTier = 3;
  ...
  if (pairCount > 0 && state.pairs && state.pairs.length > 0) {
    ...
    if (currentTier === 4) {
      tierDiscount = discountedSubtotal * 0.15;
  ...
  if (tierDiscount > 0) {
    ...
    if (currentTier === 4) {
      tierLabel.textContent = 'Tier 4 (15%):';
    } else if (currentTier === 3) {
      tierLabel.textContent = 'Tier 3 (10%):';
    } else {
      tierLabel.textContent = 'Tier 2 (5%):';
    }
  ...
  // Bonus gift (show if tier 4)
  const bonusItem = document.getElementById('tooltip-bonus-item');
  if (bonusItem) {
    bonusItem.style.display = currentTier >= 4 ? 'flex' : 'none';
  }
...
function updateStickyCart() {
  ...
  if (pairCount > 0 && state.pairs && state.pairs.length > 0) {
    ...
    if (currentTier === 4) {
      tierDiscountCents = Math.round(discountedSubtotalCents * 0.15);
      totalSavingsCents += tierDiscountCents;
      console.log('Tier 4 Discount (15%):', (tierDiscountCents / 100).toFixed(2));
    } else if (currentTier === 3) {
    ...
    if (currentTier >= 4) {
      ...
      console.log('Added Bonus Gift: €35.00');
    }
  }
  ...
  if (hasIncompleteProduct) {
    ...
    if (currentTier === 1) {
      ...
      // Change messaging for 4+ pairs
      updateIncentiveMessage(`⭐ <strong>Platinum Tier Unlocked!</strong> ${pairCount} pairs = Maximum savings! 🎉`);
    }
  }
  ...
  if (currentTier > prevState.currentTier && currentTier > 0) {
    ...
    if (currentTier === 1) {
      ...
      triggerHapticFeedback('success');
      // Enhanced confetti for tier 4!
      setTimeout(() => triggerStickyCartConfetti(), 300);
      setTimeout(() => triggerStickyCartConfetti(), 600);
    }
  ...
  if (tooltipBogo && tooltipTotal) {
    ...
    let tierDiscountCents = 0;
    if (currentTier === 4) tierDiscountCents = Math.round(subtotalAfterBogo * 0.15);
    else if (currentTier === 3) tierDiscountCents = Math.round(subtotalAfterBogo * 0.10);
    ...
    if (currentTier >= 2) {
      ...
      if (currentTier === 4) {
        tooltipTierLabel.textContent = 'Tier 4 Bonus (15%):';
      } else if (currentTier === 3) {
        tooltipTierLabel.textContent = 'Tier 3 Bonus (10%):';
      } else {
        tooltipTierLabel.textContent = 'Tier 2 Bonus (5%):';
      }
    } else {
    ...
    const tooltipBonusItem = document.getElementById('tooltip-bonus-item');
    if (tooltipBonusItem) tooltipBonusItem.style.display = currentTier >= 4 ? 'block' : 'none';
...
function updatePartitionedPricing() {
  ...
  // Calculate tier discount
  const tierPercent = pairCount >= 4 ? 0.15 : pairCount >= 3 ? 0.10 : pairCount >= 2 ? 0.05 : 0;
  ...
  console.log('Current tier:', pairCount >= 4 ? 4 : pairCount >= 3 ? 3 : pairCount >= 2 ? 2 : 1);
  ...
  // Total savings
  const totalSavingsCents = bogoDiscountCents + tierDiscountCents + shippingSavingsCents + cableSavingsCents + bonusGiftSavingsCents;
  ...
  if (tierRow) {
    if (pairCount >= 2) {
      ...
      if (tierLabel) {
        if (pairCount >= 4) {
          tierLabel.innerHTML = '<span class="discount-icon">💎</span> Tier 4 Bonus (15% OFF):';
        } else if (pairCount >= 3) {
  ...
  // Show/hide bonus gift row (Tier 4)
  const bonusRow = document.querySelector('.bonus-row');
  if (bonusRow) {
    bonusRow.style.display = pairCount >= 4 ? 'flex' : 'none';
    const bonusAmountEl = bonusRow.querySelector('.pricing-amount');
    if (bonusAmountEl && pairCount >= 4) {
      bonusAmountEl.innerHTML = `-${formatModalMoney(3500)}`;
    }
  }
...
function updateReviewProgressBar(pairCount) {
  ...
  if (pairCount >= 4) {
    segments[3]?.classList.add('filled');
    console.log('✅ Tier 4 segment filled');
  } else if (pairCount === 3) {
    segments[3]?.classList.add('partial');
    console.log('⏳ Tier 4 segment partial');
  }
  ...
  if (currentStatus && nextBenefit) {
    if (pairCount === 0) {
      ...
      currentStatus.textContent = '💎 Tier 4 PLATINUM UNLOCKED - Maximum Savings!';
      nextBenefit.textContent = '15% OFF + Premium Shipping + Bonus Cable + Bonus Gift';
    }
  }
...
function updateReviewHeaderSavings() {
  ...
  if (pairCount >= 4) {
    totalSavings += 3500; // €35.00 in cents
  }
...
class TierCelebrations {
  ...
  celebrate(tier) {
    ...
    if (tier === 2) {
      ...
      // Fallback for tier 1 or unknown
      config = {
        icon: '🎯',
        message: 'TIER 1 ACTIVATED!',
        submessage: 'BOGO Savings Unlocked!',
        breakdown: `BOGO Savings`,
    ...
    primaryBtn.onclick = (e) => {
      ...
      if ((isTier3 || isTier4) && typeof proceedToCheckout === 'function') {
        ...
        console.log(`✅ TIER ${tier}: Just closing modal`);
      }
    };
    ...
    if (tier === 2) this.triggerConfetti();
      ...
      setTimeout(() => this.triggerFireworks(), 500);
    }
  ...
  calculateTierSavings(tier) {
    ...
    const tierPercent = tier === 4 ? 0.15 : tier === 3 ? 0.10 : tier === 2 ? 0.05 : 0;
    ...
    return (bogoSavings + tierSavings + shippingSavings + cableSavings + bonusGiftSavings) / 100;
  }
